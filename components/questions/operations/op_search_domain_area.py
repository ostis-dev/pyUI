...
sc_operation_node = session.create_el_full_uri(u"/etc/operations/операция поиска области определения")[1]
sc_main_question = session.find_el_full_uri(u"/etc/questions/поиск области определения")
sc_nsm_command = session.find_el_full_uri(u"/etc/nsm/descr_nsm_cmd_dl5")
...
class Op_search_example(operation.Operation):
    ...
    def __init__(self):
        print "init: Op_search_subset"
        pass
    
    # функция проверки условия запуска
    @classmethod
    def checking(self, question):
        print "checking: Op_search_subset"
