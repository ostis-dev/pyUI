...
sc_operation_node = session.create_el_full_uri(u"/etc/operations/операция поиска утверждений")[1]
sc_main_question = session.find_el_full_uri(u"/etc/questions/поиск утверждений")
sc_nsm_command = session.find_el_full_uri(u"/etc/nsm/descr_nsm_cmd_dl12")
...
class Op_search_example(operation.Operation):
    ...
    def __init__(self):
        print "init: Op_search_statement"
        pass
    
    # функция проверки условия запуска
    @classmethod
    def checking(self, question):
        print "checking: Op_search_statement"
    ...
    def running(self, question):
        print "runing: Op_search_statement"
        res = []
