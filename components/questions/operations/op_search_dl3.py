...
sc_operation_node = session.create_el_full_uri(u"/etc/operations/отношения, области определения которых строго пересекаются с описываемым множеством")[1]
sc_main_question = session.find_el_full_uri(u"/etc/questions/поиск отношения, области определения которых строго пересекаются с описываемым множеством")
sc_nsm_command = session.find_el_full_uri(u"/etc/nsm/descr_nsm_cmd_dl3")

# класс операции поиска всех выходящих дуг
class Op_search_domain_3(operation.Operation):
    
    # функция инициализации
    def __init__(self):
        print "init: Op_search_domain_3"
        pass
    
    # функция проверки условия запуска
    @classmethod
    def checking(self, question):
        print "checking: Op_search_domain_3"
    ...
    def running(self, question):
        print "runing: Op_search_domain_3"
        res = []
