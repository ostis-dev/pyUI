...
sc_operation_node = session.create_el_full_uri(u"/etc/operations/операция поиска определения")[1]
sc_main_question = session.find_el_full_uri(u"/etc/questions/поиск определения")
sc_nsm_command = session.find_el_full_uri(u"/etc/nsm/descr_nsm_cmd_dl7")

# класс операции поиска всех выходящих дуг
class Op_search_definition(operation.Operation):
    
    # функция инициализации
    def __init__(self):
        print "init: Op_search_definition"
        pass
    
    # функция проверки условия запуска
    @classmethod
    def checking(self, question):
        print "checking: Op_search_definition"
    ...
    def running(self, question):
        print "runing: Op_search_definition"
        res = []
