...
sc_operation_node = session.create_el_full_uri(u"/etc/operations/операция поиска пояснения")[1]
sc_main_question = session.find_el_full_uri(u"/etc/questions/поиск пояснения")
sc_nsm_command = session.find_el_full_uri(u"/etc/nsm/descr_nsm_cmd_dl8")
